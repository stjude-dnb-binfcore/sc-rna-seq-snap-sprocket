...
Copy `inputs/preprocessing.example.json` to the ignored
`inputs/preprocessing.json`, replace every example path, then validate or
submit directly with Sprocket from the repository root:
